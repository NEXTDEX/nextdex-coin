...
#if defined(NDEBUG)
# error "NEXTDEX cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfcf73f1bc1c9fa00ed25f2a4f2cd199843e991e061423e8b5fba72eb083744ba");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NEXTDEX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NEXTDEX Signed Message:\n";
...
{
    // NEXTDEX: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 430 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 120400000 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NEXTDEX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NEXTDEX: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1508352015, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 181020171158 NextDex Next Digital Exchange born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 430 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a935ed73ab864d5ee689707e504a7b8713c5afe89426115361d1e88884fc6c83d8e5163af1f034a05ee626222e74b43282f52f3438ca66960608f2f213146be7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508352015;
        ...
        assert(block.hashMerkleRoot == uint256("0x2eebb53fb587adf6a4d27149f151f4031ecc8cdc4335c0698aada3e6e5edf85a"));
...
unsigned char pchMessageStart[4] = { 0x06, 0x34, 0x6e, 0xbc }; // NEXTDEX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NEXTDEXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NEXTDEXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NEXTDEXMiner(CWallet *pwallet)
{
    printf("NEXTDEXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nextdex-miner");
    ...
    try { loop {
        ...
        printf("Running NEXTDEXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NEXTDEXMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NEXTDEXMiner, pwallet));
}
